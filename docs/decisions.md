...
### False idle when every runnable slot was collecting (amendment; found via M08-T04r's `bench/largelive.c` fixture)

`finddispatchable` returning 0 has two distinct causes: the runnable queue is genuinely empty, or every runnable slot's heap was `NvHeapCollecting` at that moment (each one requeued to the tail instead of dispatched). The idle branch's `gcoutstanding == 0` check, added to decide "was a collector a factor", did not distinguish these: by the time `gcfoldall` runs, a burst of collectors from an idle-sweep pass may have already all completed and folded, driving `gcoutstanding` back to 0 while the runnable queue is provably non-empty (the very slots that were requeued are still sitting there, now genuinely dispatchable). The code as it stood then fell through to `if(!havedeadline) return NvSchedIdle;` -- reporting idle, or worse, running the deadline-wake scan below, with `nrunnable != 0`. That is exactly the false idle/deadlock the "never falsely report idle or deadlock" corollary above requires never happen, and it is empirically real, not theoretical: `bench/largelive.c` (M08-T04r), with `gcoffload` low enough that ordinary peer processes' own tiny mailbox-fragment collections went off-process, reproduced it via a `check(state == NvSchedProgress)` assertion firing with peers still runnable.

Fixed with one added check, first thing inside the `gcoutstanding == 0` branch: `if(r->nrunnable != 0) return NvSchedProgress;`. Soundness: `gcoutstanding == 0` means no exec has `offlaunched` set, so every `gcfold` call on a runnable slot returns 1 (dispatchable) unconditionally; the next `finddispatchable` call is therefore guaranteed to succeed immediately if `nrunnable != 0`, so reporting progress and letting the caller step again is both correct and prompt. No test in this repository exercised the case (every existing benchmark and CLI call site hardcodes `gcoffload = 0`, per the polarity note above, so the idle-sweep-collects-several-then-all-complete-before-the-next-dispatch-attempt race this needs never arose) until M08-T04d added one: real collector-proc timing cannot pin this deterministically (the race is between two adjacent, fast, in-process calls -- `finddispatchable`'s scan and the idle branch's `gcfoldall` -- with no syscall or existing hold point between them, so `nvschedgchold`'s park-in-the-child design cannot control it), so a second, structurally different test-only seam was added instead: `NvScheduler.gcidlestep`, a plain function-pointer field called once, right at that exact point, nil/no-op in production. `tests/memory/offloadtest.c`'s `holdfalseidle` installs it to flip every outstanding exec's heap to idle-with-success directly (under its own lock, from the scheduler proc, so nothing races it), forcing the precise interleaving deterministically. `rc bench/largelive.rc` remains real-world corroboration; `holdfalseidle` is now the deterministic regression.

### What this does not settle

The actual `gcoffload` default *value* (T04d, measurement-driven, settled in D075); whether a CLI flag exposes it before T04d (added in M08-T04d: `-o words`/`$nervous_gcoffload`, `cmd/nervous/main.c`); work stealing, migration, or any milestone-10 concern (D070 already separates these). This decision is scoped to one scheduler proc launching and reaping collector procs for its own processes' heaps.

## D075 - `gcoffload` default stays 0; the crossover is real but bracketed, not located

M08-T04d measured `bench/largelive.c`/`bench/largelive.rc` (`-o words`/`$nervous_gcoffload` added to the CLI first, since nothing before this task could force off-process collection outside the hand-built `tests/memory/offloadtest.c` scenarios) with three repeated runs per shape/threshold, after a single-run pass had already shown a result contradicting D068's founding assumption at one shape and correctly declined to act on it (`bench/README.md`, "Repeated runs (M08-T04d)" has the full table).

The repeats confirm a real, repeatable crossover: at a 50000-word owner live set, off-process collection makes the tail (p99.9/max) consistently roughly 2.2x *worse* than inline, at two different thresholds (1000 and 25000 words) that barely differed from each other -- meaning the threshold value itself was not what mattered, only whether the collection being offloaded was large enough to repay the fork. At a 500000-word live set, off-process collection makes p99.9/max consistently roughly 2x *better*. Both directions held in all three repeats of every row.

This crossover is bracketed between 50000 (off-process hurts) and 500000 words (off-process helps), not located. Because the threshold value did not distinguish the two off-process rows at the small end, this data cannot support picking any specific number in that bracket as a default -- doing so would be exactly the "choose from insufficient data" this project's benchmark discipline has repeatedly declined to do (T04p, and this same fixture's own single-run pass).

**Decision: `gcoffload` stays 0 (never off-process), the existing D074 safe default, unchanged.** Reasons: every workload this tree currently exercises (every existing test and benchmark) has a live set far smaller than even the 50000-word "hurts" bracket, so 0 costs nothing today; a guessed positive default between the brackets risks regressing every one of those workloads by roughly 2x tail for no measured benefit; and the CLI/environment knob (`-o`/`$nervous_gcoffload`) now exists for anyone who does have a genuinely large live set to opt in explicitly, without the runtime guessing on their behalf. This is the "0, because it did not repay at any measured realistic shape" outcome, and is exactly as legitimate a T04d conclusion as picking a nonzero number would have been, per this project's standing measurement discipline.

`NvGcsweepcap = 8` (the idle-sweep per-pass launch cap, D074) also stays as-is, explicitly unmeasured and provisional: this fixture's peers never grew a large enough live set to trigger the idle-sweep path at all (only the demand path, via the owner), so it was never exercised under real load by this measurement.

What would reopen this: a real workload (not a synthetic fixture) with a live set past roughly 100000 words, at which point re-measure to locate the actual crossover rather than guess a number inside the current bracket; or the persistent-collector-pool mechanism refinement (`docs/questions.md`, "Mechanism refinement noticed, not scheduled"), which would lower the fork's fixed cost and could move the crossover left of 50000 words rather than requiring a threshold guess within the current bracket.

Also settled by this task: the D074 false-idle amendment's deterministic regression test (`NvScheduler.gcidlestep`, `tests/memory/offloadtest.c`'s `holdfalseidle`) -- see that amendment's updated text above for why a second test-only seam, not an extension of `nvschedgchold`, was needed.

...
## D063 - Per-process copying collector

Each process owns one heap: a contiguous block of words with bump-pointer allocation. When an allocation does not fit, the owning process collects: a Cheney breadth-first copy of every live term into a fresh to-space, after which the old space and every adopted fragment (D064) are freed. The to-space is sized from the live data found (grow when live data exceeds half the space; never below a small minimum); this is the only sizing policy in this milestone. Collection happens only at an allocation performed by the owning process, so no process ever scans, moves, or frees another process's heap, and the scheduler never collects on a process's behalf.
...
Rationale: a copying collector's cost is proportional to live data, not to garbage, and an actor's live set is tiny (a ring node's is one pid and one token), so collections are cheap and the heap compacts itself; allocation is a pointer bump. Generational and concurrent collection are deliberately out of scope: with per-process heaps this small there is no pause problem for them to solve, and a process that is not collecting is never paused by one that is, which is the only "no unrelated process pauses" guarantee milestone 08 makes.

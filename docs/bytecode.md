...
`tuple` reads `count` consecutive registers beginning at `first`, in ascending order; it copies element words without traversing them (D061), so it never faults on term depth -- the `NvMaxtermdepth` ceiling is enforced where a term is copied across a process boundary (`send`, `return` from the root frame, `exit`, `spawn`), compared (`testeq`), or printed. `call`, `tailcall`, and `spawn` consume one argument-tuple register. `tailcall` replaces the current frame with the callee's (same caller, same result destination) and does not count against the frame limit; the compiler emits it for every source call in tail position (`semantics.md`, "Functions", and D047), so code following it within the same function is unreachable and is not emitted. `spawn` names a statically resolved function constant and writes its new PID. `self` writes the current PID, `makeref` writes a fresh Ref, and `send` asynchronously copies `message` to `pid` and writes the sent value.

`recvbegin` starts at the oldest mailbox message; `recvnext` advances past the current candidate. Both write the candidate (or atom `'undefined`) and a `'true`/`'false` found flag, so both destinations are initialized on every path. Since D064 the candidate is a term referring to the message's mailbox fragment in place, not a copy; `recvtake` consumes the current candidate after compiled pattern tests succeed and adopts its fragment into the process heap, so terms bound from it stay valid. `recvwait` requires an exhausted scan, changes the process to waiting, and resumes at `retry-target` after a send wakes it. Candidate pattern registers are tentative and are used only on their clause success edge. `exit` terminates the process with the arbitrary term in `reason`; it has no fallthrough.
...
`guard` enters guard mode (D060): until the matching `guardend`, any fault the executing instruction would raise instead transfers control to `fail-target` and leaves guard mode; `guardend` leaves it normally. Guard mode is one field of the execution state, not a stack, and a quantum boundary inside a guard preserves it. `istype` writes `'true` to `dst` if `src` holds a term of `kind` (an immediate: 0 integer, 1 atom, 2 tuple, 3 pid, 4 ref, the `nvtermkind` numbering in `include/nvvm.h`) and `'false` otherwise; it never faults.
...
D071 additionally verifies guard regions before register-flow analysis, including unreachable instructions. `guard`/`guardend` pairs are lexical and non-nested. Only value operations (`loadk`, `move`, `tuple`, `getelem`), arithmetic and comparisons, pattern tests, `istype`, `nop`, jumps within the region, explicit `fail`, and the closing `guardend` are permitted inside. In particular, no call, tail call, return, process instruction, or I/O instruction may run with guard mode active.

Every ordinary branch preserves the active guard identity. Only falling through `guard` enters its body, and only falling through `guardend` leaves it normally. Branches within a guard may target its own `guardend`; branches from outside may target the `guard` instruction or skip the entire region, but must not enter its body or close. A guard's exceptional failure target must be outside every active region; it may be another `guard` instruction. Loops entirely within a region are permitted in raw bytecode. These checks protect the execution-wide failure target from being carried into another frame and keep the failure edges used by register-flow analysis sound.

...
### First real run (after the D074 false-idle fix)

User ran `rc bench/largelive.rc` after `mk tests benchmarks`; all five cases completed without error (the crash from the previous session's run, and the `lib/sched.c` fix for it, are recorded above and in `docs/decisions.md`'s D074 amendment). `rc tests/run.rc` and `nervous_gcstress=1 rc tests/run.rc` both passed in full immediately after, reconfirming the scheduler fix did not regress anything. Single run, not a confidence interval, exactly like every other section in this file.

| retainwords | gcoffload | baseline p50/p99.9/max (ns) | loaded p50/p99.9/max (ns) | owner's own collections | owner last live words |
|---|---|---|---|---|---|
| 50000 | 0 (inline) | 1239 / 6118 / 43906 | 14713 / 205051 / 317023 | 196 | 50009 |
| 50000 | 1000 (off-process, owner only) | 852 / 3872 / 48475 | 15023 / 437591 / 727589 | 139 | 50009 |
| 50000 | 25000 (off-process, owner only) | 852 / 4027 / 40267 | 2478 / 452149 / 746794 | 89 | 50009 |
| 500000 | 0 (inline) | 852 / 3484 / 36473 | 14404 / 47856 / 4255420 | owner already exited (rootstate Done) | n/a |
| 500000 | 1000 (off-process, owner only) | 852 / 3871 / 72635 | 852 / 22224 / 4139035 | 5 | 500018 |

Read plainly, without over-concluding from one run each:

- The `baseline`-to-`loaded` p50 shift (roughly 852-14700 ns at 50000 words) is present at every `gcoffload` value, including the off-process ones, matching this section's earlier prediction: it is FIFO quantum-sharing with a continuously-runnable owner (D059), not GC, since offloading a collection does not change how many of the owner's own ordinary quanta a peer waits behind. The 25000 row's unusually low loaded p50 (2478 ns) does not fit that pattern and is flagged, not explained, below.
- **Surprising result, worth stating plainly rather than glossing over:** at retainwords=50000, both off-process thresholds made the *tail* worse than inline, not better -- p99.9/max roughly doubled (205051/317023 inline vs 437591/727589 and 452149/746794 off-process). This is the opposite of what D068's rationale ("fork cost is a fixed few tens of microseconds, noise next to the pause it replaces") predicts at this live-set size, in this one run, on this one machine. At retainwords=500000, off-process instead improved p99.9 (47856 to 22224) while `max` stayed comparably enormous either way (~4.1-4.3 ms) -- so the picture is not even consistent across the two shapes measured so far.
- The owner's own collection count differs sharply across `gcoffload` within the SAME 40000-round-trip measurement window (196 inline vs 139 vs 89 off-process at 50000 words), meaning the owner made less forward progress per wall-clock time under off-process collection in this run, not more -- another data point against "fork cost is noise" rather than for it, though it could equally be single-run OS scheduling variance now that real concurrent Plan 9 procs are actually involved (the inline path has none).
- This is exactly the kind of evidence `docs/questions.md`'s "Mechanism refinement noticed, not scheduled" note said would be the concrete trigger to revisit a persistent-collector-pool: not confirmed yet (one run each, one machine), but no longer purely hypothetical either.

**Not yet a basis for choosing a `gcoffload` default.** Before T04d acts on this: repeat each row multiple times to separate a real effect from single-run OS scheduling noise (now that off-process cases involve genuine concurrent Plan 9 procs, unlike the fully deterministic inline path); consider whether the p50 anomaly at gcoffload=25000 is a hint about interleaving with collection frequency rather than pure chance; and measure a few more `retainwords`/`gcoffload` combinations between and around 50000 and 500000 before treating either direction (off-process helps at 500000, hurts at 50000) as a real crossover rather than noise at both ends.
